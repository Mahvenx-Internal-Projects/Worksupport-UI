...
const BOT_FLOW = {
  ...
  contact: {
    text: "What's the best email to reach you on? (Type it or press Skip to use your account email)",
    allowSkip: true,
    next: 'describe',
  },
  describe: {
    text: "Briefly describe your issue — the more detail, the faster we can help:",
    next: 'submit',
  },
};
...
const SupportChatWidget: React.FC = () => {
  ...
  const handleUserText = useCallback(async (text: string) => {
    ...
    if (stage === 'bot') {
      if (step === 'contact') {
        ...
        const email = isSkip ? (user?.email || '') : text;
  ...
  }, [stage, step, botData, ticketId, user, botSay, addMsg]);
  ...
  const submitTicket = useCallback(async (data: typeof botData) => {
    ...
    try {
      const res = await api.post('/support/tickets', {
        ...
        contactEmail: data.email || user?.email || undefined,
        contactPhone: undefined,
        firstMessage: data.description || data.topic,
      });
      ...
      removeTyping(typId);
      addMsg('bot', "⚠️ Couldn't create ticket. Please email **help@worksupport360.com** or WhatsApp **+91-9441363687**.");
      setStage('idle');
    }
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50" style={{ fontFamily: "'Outfit', sans-serif" }}>
      {/* Chat window */}
      {open && (
        <div className="absolute bottom-16 right-0 flex flex-col bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden"
          style={{ width: 360, height: 480, animation: 'slideUp .3s cubic-bezier(.16,1,.3,1)' }}>
