        ...
        {selected && (
          <div style={{...card,position:'sticky',top:72,maxHeight:'calc(100vh - 100px)',overflow:'hidden',display:'flex',flexDirection:'column'}}>
            ...
            <div style={{flex:1,overflowY:'auto',padding:'16px'}}>
              ...
              {/* Public applicants from homepage */}
              {(selected.publicApplicants??[]).length>0&&(
                <div style={{marginBottom:14}}>
                  <div style={{fontSize:11,fontWeight:700,color:'#64748b',textTransform:'uppercase' as const,letterSpacing:'0.05em',marginBottom:8}}>
                    📩 Public Applicants ({(selected.publicApplicants??[]).length})
                  </div>
                  <div style={{display:'flex',flexDirection:'column',gap:7}}>
                    {(selected.publicApplicants??[]).map((a:any,i:number)=>(
                      <div key={i} style={{padding:'11px 13px',borderRadius:11,background:'#f0fdf4',border:'1px solid #86efac'}}>
                        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:6}}>
                          <div style={{fontWeight:700,fontSize:13,color:'#0f172a'}}>{a.applicantName||a.name}</div>
                          <a href={`tel:${a.applicantMobile||a.mobile}`} style={{display:'inline-flex',alignItems:'center',gap:5,padding:'4px 11px',borderRadius:8,background:'#059669',color:'#fff',textDecoration:'none',fontSize:12,fontWeight:700}}>
                            📱 {a.applicantMobile||a.mobile||'—'}
                          </a>
                        </div>
                        <a href={`mailto:${a.applicantEmail||a.email}`} style={{fontSize:12,color:'#6366f1',textDecoration:'none',display:'block',marginTop:3}}>{a.applicantEmail||a.email}</a>
                        {(a.coverNote||a.note)&&<div style={{fontSize:12,color:'#374151',marginTop:5,fontStyle:'italic',background:'#fff',padding:'5px 9px',borderRadius:7}}>"{a.coverNote||a.note}"</div>}
                        <div style={{fontSize:10,color:'#94a3b8',marginTop:4}}>Applied: {new Date(a.appliedAt||a.createdAt||Date.now()).toLocaleDateString('en-IN',{day:'numeric',month:'short'})}</div>
                      </div>
                    ))}
                  </div>
                </div>
